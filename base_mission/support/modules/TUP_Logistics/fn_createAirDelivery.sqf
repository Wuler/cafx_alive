...
for "_i" from 0 to (_num-1) do
{
	...
	if (count _itemArray > 5) then {
		_container = "Land_Cargo20_military_green_F";
	} else {
	...
	if (_prefDel == 1) then { // Setup Airlift object
		_v FlyInHeight 400; // 600 for c130
	...
	if (_prefDel == 3) then { // Setup GPS paradrop
		_object =  createVehicle ["Land_CargoBox_V1_F",[0,0,0],[],0,"CAN_COLLIDE"];
		_v FlyInHeight 600; // 600 for c130
	};
	...
	[_v,_itemArray,_pos, _player, _prefDel,_object] spawn {
        ...
        waitUntil {sleep 30; !(_grp call CBA_fnc_isAlive) || (damage _v > 0.6) || (_v distance _pos < 2000)};
		...
		if (_prefDel != 3) then { // Flyin low for paradrop and airlift, for gps guided drop stay high
			_v FlyInHeight (100 + (random 50)); 
		} else {
			_v FlyInHeight (450 + (random 100)); // (450 + (random 100)); for c130
		};
		
        waitUntil {sleep 10; !(_grp call CBA_fnc_isAlive) || (damage _v > 0.6) || (_v distance _pos < 600)};
