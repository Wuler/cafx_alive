...
if (tup_logistics_delivery_sel == 2) then {
	...
	_msg = "Click on Map to choose delivery location. Helicopters require a helipad within 500m. Other aircraft will land at nearest runway.<br/><br/> Paradrop is a highly inaccurate delivery method. Expect items to land anywhere from 100m-750m from the chosen destination position.<br/><br/>Airlift does not require a helipad or runway, but please ensure a secure open space is chosen for the landing zone.<br/><br/>GPS guided para-drop should land within 100m of your chosen position."
};
