...
/*	if (!(isNil "tup_logistics_nextorder") && (date < tup_logistics_nextorder)) then {
	...
	if ( (({!((_x select 1) iskindof "ReammoBox" || (_x select 1) iskindof "ReammoBox_F" || (_x select 1) iskindof "StaticWeapon")} count tup_logistics_order) > 0) && (tup_logistics_delivery_sel == 3) ) then {
	...
	if ( (({(_x select 1) in [
		...
		"ACRE_OE_303"];} count tup_logistics_order) > 0) && (tup_logistics_delivery_sel == 2) ) then {
