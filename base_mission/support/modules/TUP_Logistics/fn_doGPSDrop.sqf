...
sleep 1;
_pilot = (group _Transporter) createUnit ["B_Helipilot_F", _pos, [], 0, "CAN_COLLIDE"];
[_pilot] join grpNull;
_pilot allowDamage false;

_Parachute = createVehicle ["Steerable_Parachute_F", position _obj,[],0,"CAN_COLLIDE"];
...
[_obj,_Parachute, _Marker, _pos] spawn {
	...
	deleteVehicle _pilot; //(gunner _Parachute);
		
	waitUntil {sleep 0.5; ((getPosATL _obj select 2) < 5) || !(_Parachute call CBA_fnc_isAlive) };
	...
	if (debug_mso) then {
		...
		_objm = [_name, position _obj, "Icon", [1,1], "TEXT:", "OBJ", "TYPE:", "mil_dot", "COLOR:", "ColorBlue", "GLOBAL"] call CBA_fnc_createMarker;
	};
