...
_weaponarr = ["hgun_Rook40_F","hgun_P07_F","hgun_ACPC2_F"];
...
if (_check) exitWith {
	while {true} do {
		{if (((side _x) == civilian) && !(isplayer _x)) then {_civarray set [(count _civarray), _x]}} forEach allUnits;
...
if (!(_check) && ((round random 100)> _chance)) exitWith {
