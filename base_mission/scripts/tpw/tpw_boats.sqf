...
if (isMultiplayer) then {
	...
	tpw_boat_num = (paramsarray select 47);
};

tpw_boat_active = false; // Global enable/disable
if ((paramsarray select 48) == 1) then {
	tpw_boat_debug = true;
} else {
	tpw_boat_debug = false;
};
