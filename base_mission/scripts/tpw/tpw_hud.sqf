...
tpw_hud_fnc_prx = 
	{
	private ["_civ","_friendly","_enemy","_squad","_unitarray","_side","_ctf"];
	_unitarray = [];
	_civ = 0;
	_friendly = 0;
	_enemy = 0;
	...
	["tpw_hud","oneachframe",
		...
		for "_ctj" from 0 to (count tpw_hud_visible - 1) do	
			...
			if (!isnil "_unit" && {!isnull _unit}) then 
				...
				// Unit icon
				drawIcon3D [(_unit getvariable ["tpw_hud_visicon",""]),(_unit getvariable ["tpw_hud_unitcolour",[0,0,0,0]]), [_xpos,_ypos,_zpos], (_unit getvariable ["tpw_hud_scale",0]), (_unit getvariable ["tpw_hud_scale",0]),0,(_unit getvariable ["tpw_hud_distance",""]),0,tpw_hud_unittextsize , "PuristaBold"]; 
