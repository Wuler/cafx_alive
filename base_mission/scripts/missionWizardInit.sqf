...
private ["_isServer","_wizard","_login","_missionCenter","_missionCenterTrigger","_playersNumber","_difficulty","_totalEnemyUnits","_isCQB","_objType","_objArray","_check","_objFirstTime",
		 ...
		 "_preciseMarkers","_reinforcement","_artillery","_logout"];
         
_isServer = if (serverCommandAvailable "#logout" || isServer) then {true} else {false}; 
_wizard = name (player);

if !(mcc_missionmaker == "" || mcc_missionmaker == _wizard || _isServer) exitwith {hintsilent "MCC in use!"};

if !(mcc_missionmaker == _wizard) then {
	_login = [] execVM "scripts\mcc_login.sqf";
	waituntil{scriptdone _login};
	if (dialog) then {closeDialog 0};
}; 

MCC_MWisGenerating = true;

mcc_faction = ["caf_ag_me_t","caf_ag_afr_p","Kae_TK"] call bis_fnc_selectRandom;
mcc_sidename = "EAST";

//Get params
_playersNumber 		= count (call CBA_fnc_players);
_difficulty 		= 1 + (round (random 2));
_stealth 			= false;						
_isIED 				= if (mcc_faction != "Kae_TK") then {[true,false] call BIS_fnc_selectRandom} else {false};	
_isSB 				= if (mcc_faction != "Kae_TK") then {[true,false] call BIS_fnc_selectRandom} else {false};					
_isAS				= if (mcc_faction != "Kae_TK") then {[true,false] call BIS_fnc_selectRandom} else {false};				
_isRoadblocks		= [true,false] call BIS_fnc_selectRandom;
_armor 				= if (mcc_faction == "Kae_TK") then {[true,false] call BIS_fnc_selectRandom} else {false};	
_vehicles 			= if (mcc_faction != "Kae_TK") then {[true,false] call BIS_fnc_selectRandom} else {false};			
_weatherChange		= false;
_wholeMap			= true;
MW_debug			= false;
_preciseMarkers		= [true,false] call BIS_fnc_selectRandom;
_reinforcement		= round (random 2);
_artillery			= if (mcc_faction == "Kae_TK") then {round (random 2)} else {round (random 1)};
_isCQB 	=  if (mcc_faction == "Kae_TK") then {false} else {true};
_isCiv	=  false;
...
if (isnil "_faction") exitWith 
{
    ...
    _logout = []execVM "scripts\mcc_login.sqf";
};

//Build the faction's unitsArrays and send it to the server.
_checkunits = [] call MCC_fnc_MWCreateUnitsArray;
waituntil {_checkunits};	
...
if (_wholeMap) then
{
	...
	{
		hsim_worldArea = createtrigger ["emptydetector",position center];
		hsim_worldArea settriggerarea [4000,4000,0,true];
	};
	...
	{
		MCC_MWcityLocations     = [getpos MWMissionArea,2500,"city"] call MCC_fnc_MWbuildLocations;
		MCC_MWmilitaryLocations = [getpos MWMissionArea,2500,"mil"] call MCC_fnc_MWbuildLocations;
		MCC_MWhillsLocations 	= [getpos MWMissionArea,2500,"hill"] call MCC_fnc_MWbuildLocations;
		MCC_MWnatureLocations 	= [getpos MWMissionArea,2500,"nature"] call MCC_fnc_MWbuildLocations;
		MCC_MWmarineLocations	= [getpos MWMissionArea,2500,"marine"] call MCC_fnc_MWbuildLocations;
	...
	//Find mission center
	_center = [position center,10,_isCQB,MCC_MWBasedLocations] call MCC_fnc_MWFindMissionCenter;
...
for [{_x = 1},{_x <=3},{_x = _x+1}] do		
{
	...
	if (_objType != "None") then 
	{
		...
		switch (_objType) do
		{
		  ...
		  case "Kill HVT":		
			{
				[[_objPos, _isCQB, false, _side, _faction, _sidePlayer, _factionPlayer ,_preciseMarkers], "MCC_fnc_MWObjectiveHVT", false, false] call BIS_fnc_MP;
			};
